...
const WalletButton = () => {
  ...
  return (
    ...
    >
      <button
        className="flex cursor-pointer items-center gap-[0.375rem] bg-transparent text-[0.75rem] font-bold leading-3 text-white before:h-[1.375rem] before:content-clip"
        onClick={() => {
          if (!walletConnected) {
      ...
      {userInfo?.address && isAddressShown && (
        <div className="text-gray absolute right-0 mt-1 rounded-[0.375rem] bg-white p-4 drop-shadow-wallet before:absolute  before:left-0  before:right-0 before:top-[-1rem] before:w-4 before:content-tooltip max-md:left-0  max-md:right-0  max-md:top-[1.875rem] max-md:mx-auto max-md:my-0 max-md:text-center">{`${String(
