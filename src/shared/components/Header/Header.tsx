  ...
  return (
    <header
      className={cn('base-wrapper bg-red max-lg:relative max-lg:w-screen', {
        'z-[999] min-h-screen pt-36': menuIsOpen,
      })}
    >
      <div
        className={cn('max-w-480 flex w-full items-center justify-between px-0 py-8 max-xl:flex-col max-xl:gap-7', {
          'max-lg:items-start': !menuIsOpen,
        })}
      >
        {!menuIsOpen && <Logo />}
        {activeWalletCardanoKey ? (
          <div className={cn('flex max-lg:flex-col max-lg:gap-10', { 'max-lg:hidden': !menuIsOpen })}>
      ...
      <Image
        ...
        src={`/icons/${menuIsOpen ? 'close' : 'menu'}.svg`}
        alt="close icon"
        width={50}
        height={50}
