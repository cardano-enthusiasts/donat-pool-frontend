    ...
    <Service>
      ...
      <div className="grid gap-8">
        <DropdownSection title="What is Donat.Pool?">
          {getSection(
            <>
              ...
              {getUl(['to start fundraising projects', 'to donate ADA funds to support projects'])}
        ...
        <DropdownSection title="How this service is different from other donation services?">
          {getSection(
            <>
              ...
              {line}
              <div>
                {getSubtitle('Transparency and privacy')}
        ...
        <DropdownSection title="How to register?">
          {getSection(
            <>
              The only thing you need to do is to connect your Nami wallet to Donat.Pool. Connection means that you
              allow Donat.Pool to call your Nami wallet and get information about your wallet balance. It&#39;s
              necessary for building transactions, and nobody can manage your wallet funds without your personal
              signature to confirm each transaction - only you can sign and submit the transaction.
              <AlertNote>
                Donat.Pool service is available only for Chrome, Chromium, Brave and Edge browsers with the installed
          ...
          {getSection(
            <>
              ...
              <AlertNote>
                ...
                or your project funds, so you have to put 4 ADA to this storage temporarily. You will get these 4 ADA
        ...
        {basicSection.map(({ title, description }) => (
          <DropdownSection title={HTMLReactParser(title) as string} key={title}>
            {getSection(HTMLReactParser(description))}
          </DropdownSection>
        ))}
      </div>
    </Service>
